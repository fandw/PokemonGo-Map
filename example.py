...
def time_left(ms, c):
    s = ms / 1000
    s = s + c.hour * 3600 + c.minute * 60 + c.second
    m, s = divmod(s, 60)
    h, m = divmod(m, 60)
    return h, m, s
...
def main():
    ...
    while steps < steplimit**2:
        ...
        for poke in visible:
            ...
            c = datetime.now()
            time_hidden = '%d:%d:%d ' % time_left(time_to_hidden, c)
            label = '%s [will be gone at %s]' % (pokemonsJSON[poke.pokemon.PokemonId - 1]['Name'], time_hidden)
...
def fullmap():
    pokeMarkers = []
    way_too_common = {10, 11, 13, 14, 16, 19, 21, 41, 96}
    for pokemon in pokemons:
        if pokemon[0] in way_too_common:
            continue
